...
class _Command(object):
    ...
    def _zdaemon_command(self, zdaemon_command, conf_file):
        ...
        pid = self._execute_zdaemon_command(
            ["zdaemon", "-C", conf_file, zdaemon_command]
        )
    ...
    def _get_config_mod(self):
        ...
        marker_path = os.path.normpath(
            os.path.join(self.config_dir, self._config_marker)
        )
...
class Init(_Command):
    ...
    def run(self):
        ...
        config_py_template = config_py_template.format(
            INSTANCE_SECRET=uuid.uuid4().hex, INSTANCE_UNIQUE=uuid.uuid4().hex
        )
...
class Start(_Command):
    ...
    def prepare_config(self):
        ...
        if missing:
            noun, verb = ("file", "exists") if len(missing) == 1 else ("files", "exist")
            ...
            zdaemon_conf = zdaemon_conf.format(
                config_dir=self.config_dir, is_https=is_https
            )
