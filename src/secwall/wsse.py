...
wsse_username_token_path = (
    "/soapenv:Envelope/soapenv:Header/wsse:Security/wsse:UsernameToken"
)
wsse_username_token_xpath = etree.XPath(
    wsse_username_token_path, namespaces=wss_namespaces
)

wsse_username_path = (
    "/soapenv:Envelope/soapenv:Header/wsse:Security/wsse:UsernameToken/wsse:Username"
)
wsse_username_xpath = etree.XPath(wsse_username_path, namespaces=wss_namespaces)

wsse_password_path = (
    "/soapenv:Envelope/soapenv:Header/wsse:Security/wsse:UsernameToken/wsse:Password"
)
...
wsse_password_type_xpath = etree.XPath(
    wsse_password_type_path, namespaces=wss_namespaces
)
...
wsu_username_created_path = (
    "/soapenv:Envelope/soapenv:Header/wsse:Security/wsse:UsernameToken/wsu:Created"
)
wsu_username_created_xpath = etree.XPath(
    wsu_username_created_path, namespaces=wss_namespaces
)
...
class WSSE(object):
    ...
    def validate(self, soap, config):
        ...
        if wsse_password:
            wsse_password, wsse_password_type = self._replace_username_token_elem(
                soap, wsse_password, "Type"
            )

        wsse_nonce = wsse_nonce_xpath(soap)
        if wsse_nonce:
            wsse_nonce, wsse_encoding_type = self._replace_username_token_elem(
                soap, wsse_nonce, "EncodingType"
            )
        ...
        if not wsse_password_type in supported_wsse_password_types:
            msg = "Unsupported password type=[{0}], not in [{1}]".format(
                wsse_password_type, supported_wsse_password_types
            )
        ...
        if config["wsse-pwd-reject-empty-nonce-creation"]:
            ...
            token_created = datetime.strptime(
                wsu_username_created, soap_date_time_format
            )

            elapsed = now - token_created

            if (
                config["wsse-pwd-reject-stale-tokens"]
                and elapsed.seconds > config["wsse-pwd-reject-expiry-limit"]
            ):
                self.on_username_token_expired(config, elapsed, soap)

        if config.get("wsse-pwd-password-digest"):
            expected_password = self._get_digest(
                config["wsse-pwd-password"], wsse_nonce, wsu_username_created
            )
        else:
        ...
        if self.check_nonce(
            wsse_nonce, now, config.get("wsse-pwd-nonce-freshness-time")
        ):
