...
def on_ssl_cert(url_config, client_cert, field_prefix, needs_auth_info=True):
    ...
    if client_cert:
        ...
        if not config_fields:
            ...
            cert_fields = dict(
                (elem[0][0].encode("utf-8"), elem[0][1].encode("utf-8"))
                for elem in subject
            )
            ...
            for config_field, config_value in list(config_fields.items()):
                ...
                if needs_auth_info:
                    auth_result.auth_info = dict(
                        (quote_plus(k), quote_plus(v)) for k, v in cert_fields.items()
                    )
...
def on_basic_auth(env, url_config, needs_auth_info=True):
    ...
    result = _on_basic_auth(
        env.get("HTTP_AUTHORIZATION", ""), username, url_config["basic-auth-password"]
    )
    ...
    if is_success:
        if needs_auth_info:
            auth_result.auth_info = {
                b"basic-auth-username": quote_plus(username).encode("utf-8")
            }
    else:
...
class _RequestApp(object):
    ...
    def __init__(self, config=None, app_ctx=None):
        ...
        for url_pattern, url_config in self.config.urls:
            ...
            url_config["from-client-ignore"][:] = list(
                map(str.upper, url_config.get("from-client-ignore", {}))
            )
    ...
    def __call__(self, env, start_response, client_cert=None, client_cert_der=None):
        ...
        ctx = InvocationContext(
            self.instance_name, self.instance_unique, next(self.msg_counter), self.now()
        )
        ...
        for c, url_config in self.urls:
            ...
            if match:
                ...
                return self._on_request(
                    ctx, start_response, env, url_config, client_cert, match
                )
        else:
    ...
    def _on_request(
        self, ctx, start_response, env, url_config, client_cert, match=None
    ):
        ...
        if url_config.get("ssl"):
            ...
            if url_config.get("ssl-cert") and not client_cert:
                return self._401(
                    ctx, start_response, self._get_www_auth(url_config, "ssl-cert")
                )
                ...
                if config_type in url_config:
                    ...
                    if not auth_result:
                        return self._401(
                            ctx,
                            start_response,
                            self._get_www_auth(url_config, config_type),
                        )
                    break
            else:
    ...
    def _on_ssl_cert(
        self, env, url_config, client_cert, data, field_prefix="ssl-cert-"
    ):
    ...
    def _compute_digest_auth_response(
        self,
        expected_username,
        expected_realm,
        expected_password,
        ...
        ha1 = hashlib.md5()
        ha1.update(
            "{0}:{1}:{2}".format(expected_username, expected_realm, expected_password)
        )
    ...
    def _on_custom_http(self, env, url_config, *ignored):
        ...
        expected_headers = {}
        expected_headers_keys = (
            header for header in url_config if header.startswith(prefix)
        )
        ...
        for key in expected_headers_keys:
            ...
            expected_headers[str(key)] = str(
                env.get("HTTP_" + key.split(prefix)[1].upper().replace("-", "_"), "")
            )
    ...
    def _on_xpath(self, unused_env, url_config, unused_client_cert, data):
        ...
        prefix = "xpath-"
        expressions = [
            url_config[header] for header in url_config if header.startswith(prefix)
        ]
        ...
        try:
            try:
                if hasattr(self.socket, "getpeercert"):
                    ...
                    cert = None
                    cert_der = None
                self.result = self.application(
                    self.environ, self.start_response, cert, cert_der
                )
                ...
                if not self.response_length:  # pragma: no cover
                    self.start_response(
                        pywsgi._INTERNAL_ERROR_STATUS, pywsgi._INTERNAL_ERROR_HEADERS
                    )  # pragma: no cover
...
class HTTPProxy(wsgi.WSGIServer):
    ...
    def __init__(self, config, app_ctx):
        ...
        super(HTTPProxy, self).__init__(
            ...
            wsgi_request_app(config, app_ctx),
            log=config.log,
        )
...
class HTTPSProxy(pywsgi.WSGIServer):
    ...
    def __init__(self, config, app_ctx):
        ...
        super(HTTPSProxy, self).__init__(
            (config.host, config.port),
            wsgi_request_app(config, app_ctx),
            log=config.log,
            handler_class=wsgi_request_handler,
